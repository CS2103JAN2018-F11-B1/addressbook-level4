...
When an `ImportCommand` is called, the corresponding CSV file will be processed.
All the information will be stored in a list of `Person` object.
Then all `Persons` in the list will be added to CRM Book one by one automatically like what a `AddCommand` can do but without manual typing.
...
[none]
* 1a. User enters an invalid file path.
+
[none]
