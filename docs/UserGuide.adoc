...
* In v 1.2: Able to add leads into CRM book and convert leads into contacts. All commands can be performed using its respective aliases. Able to start process of LinkedIn Integration. Able to generate a color for the tags added. Able to add/remove/edit remark of a person.

* In v 1.3: Introduce calander for meetings. Able to detect a duplicate based on a phone number. Able to fully authenticate linkedIn, and link contacts to linkedIn. At least two alias commands will be completed. Auto-Complete of commands enabled. Able to sort all persons in list by name alphabetically.

* In v 1.4: Able to book meetings using the calender. Able to detect a duplicate based on a email address. Able to list out all Leads at user's request. CRM Book will alert you if a contact switches companies. At least two alias commands will be completed. Able to change theme statically. Able to bulk import.
...
Format: `rem [INDEX] r/[KEYWORD]`, `remark [INDEX] r/[KEYWORD]`
...
=== Import a csv file to the CRM Book: `i`, `import`  [since v1.4]

Add all persons in the csv file to the CRM Book in one time. +
Format: `i PATH`, `import PATH`

****
* The path refers to the path of the csv file the user wants to import.
...
* `import ./sample.csv` +
Imports the file `sample.csv` in the current directory to the CRM Book.
